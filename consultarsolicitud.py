...
@consultasolicitud_api.route("/api/consultarsolicitudgeneral",methods=['POST','GET'])
def consultarsolicitudgeneral():
    ...
    try:
       ...
       if aerror == False:
          ...
          if mycursor.rowcount == 0:
             ...
             sql = "select sum(solicit.deudatotal) as monto,monthname(solicit.fecha_crea) as mes from solicit group by month(fecha_crea) limit 30"
             mycursor.execute(sql)
             grafico = mycursor.fetchall()
             listavalor = []
             listames = []
...
@consultasolicitud_api.route("/api/eliminarsolicitud",methods=['POST','GET'])
def eliminarsolicitud():
    ...
    try:
       ...
       if aerror == False:
          ...
          conectar = conectUserDatabase(row['parent'])
          mycursor = conectar.cursor(dictionary=True)
          sql = "delete from solicit where id = "+"'"+str(row['id'])+"'"
          mycursor.execute(sql)
