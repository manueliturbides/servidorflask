
#user = 'gerald'
#database = 'finanzas1'
#host = 'database-3.cm2q0e3sd7mo.us-east-1.rds.amazonaws.com'
#password = '00100267590'
#passwordhost = 'mI00100267590'


#puser = 'gerald'
#pdatabase = 'prestamo1'
#phost = 'database-3.cm2q0e3sd7mo.us-east-1.rds.amazonaws.com'
#ppassword = '00100267590'
#ppasswordhost = 'mI00100267590'


####configuracion de general database de prestaquik
puser = 'root'
pdatabase = 'general'
phost = 'general.c78ou26kqg7e.us-east-1.rds.amazonaws.com'
#phost = '127.0.0.1'
ppassword = '00100267590'
passwordhost = '00100267590'


####configuracion de basededatos usuarios database de prestaquik
zuser = 'root'
Zhost = 'general.c78ou26kqg7e.us-east-1.rds.amazonaws.com'
#Zhost = '127.0.0.1'


#puser = 'miturbides'
#pdatabase = 'general'
#phost = '127.0.0.1'
#ppassword = '00100267590'
#passwordhost = '00100267590'


#puser = 'miturbides'
#pdatabase = 'prestamo1'
#phost = '127.0.0.1'
#ppassword = '00100267590'
#ppasswordhost = 'mI00100267590'
