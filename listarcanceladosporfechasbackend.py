...
listarcanceladosporfechasbackend_api = Blueprint('listarcanceladosporfechasbackend_api',__name__)
...
@listarcanceladosporfechasbackend_api.route("/api/listarcanceladosporfechas",methods=['POST','GET'])
def listarcanceladosporfechas():
    ...
    try:
       ...
       if aerror == False:
          ...
          if len(row['sucursal']) == 0 or row['sucursal'] == None or row['sucursal'] == '':
             ...
             concat(plazo,' ',periodos) as plazo, format(interes,2) as interes,status from prestamo where status = 'C' and fecha between "+"'"+row['fechadesde']+"' and "+"'"+row['fechahasta']+"'"
          else:
             sql = "select noprest as id,date_format(fecha,'%d-%m-%Y') as fecha,nombres,apellidos,format(cantsol,2) as cantsol,\
             concat(plazo,'',periodos) as plazo,format(interes,2) as interes,status from prestamo where status = 'C' and fecha between "+"'"+row['fechadesde']+"' and "+"'"+row['fechahasta']+"'"
          ...
          if mycursor.rowcount != 0:
             salida["prestamocancelados"] = miprestamo
